...
const leerURL = () => {
  ...
  const filtros = { n: parametros.get('n') ?? '' };
...
const urlFicha = (id) => `${location.pathname}?n=${encodeURIComponent(id)}`;
...
const pintarListado = (filtros) => {
  ...
  if (!visibles.length) {
    const sugerencias = [['114/2025', 'un número'], ['resolución', 'un tipo'], ['fct', 'una materia']]
      ...
      .join(' · ');
    vacio.innerHTML = `
      <p class="vacio__titulo">Ninguna norma coincide${filtros.q ? ` con «${escapar(filtros.q)}»` : ''}.</p>
...
const pintarFicha = (norma, volverA) => {
  const traer = (ids) => ids.map((id) => datos.porId.get(id)).filter(Boolean);
  const grupos = [
    ['Derogada por', datos.derogadaPor.get(norma.id) ?? []],
    ['Modificada por', traer(norma.modificadaPor)],
...
const pintar = () => {
  ...
  if (norma) {
    ...
    vistaListado.hidden = true;
    vistaFicha.hidden = false;
    vistaFicha.innerHTML = pintarFicha(norma, ultimoListado ?? location.pathname);
